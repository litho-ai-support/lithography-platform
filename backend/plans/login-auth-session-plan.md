...
- 状态：进行中
- 创建日期：2026-08-25
- 当前阶段：主链路代码已实现；原始验收对照发现剩余编码缺口，转入
  `login-auth-session-code-completion-plan.md`；尚未开始独立验收
- 建议分支：`feat/login`
...
- 跨角色访问不得渲染无权页面：`ENGINEER`/`CUSTOMER` 互访回到自己的页面；
  前端守卫按 `docs/development/task-acceptance.md` 与 `backend/docs/api/auth-session-current.md`
  实现 `SUPER_ADMIN` 继承（可访问三个角色页），与后端受保护接口继承语义待真实核验。
...
- 全局会话失效处理当前只覆盖统一入口 `executeGraphQL`；若业务改用 Apollo hooks 直连，失效信号不会经过该链路。
...
- 负责人决定跨角色访问统一显示 403。
- 业务改用 Apollo hooks 直连时，需为 hooks 链路补齐统一的会话失效处理。
