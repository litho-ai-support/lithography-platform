// src/adapters/api/graphql/admin-document-database/validate-admin-document-database-input.decorator.ts
 ...
 * `DomainError(ADMIN_DOCUMENT_DATABASE_ERROR.INVALID_PARAMS)`，经全局过滤器
 * `mapDomainErrorToGqlCode` 的显式映射，在 production 仍稳定返回
...
export const ValidateAdminDocumentDatabaseInput = () =>
  UsePipes(
    new ValidationPipe({
      whitelist: true,
      ...
      exceptionFactory: (errors: ValidationError[]) => {
        ...
        return new DomainError(ADMIN_DOCUMENT_DATABASE_ERROR.INVALID_PARAMS, message);
      },
    }),
  );
