...
// 路径常量、类型展示口径与数据访问方法。application 层 hooks 与 Mock 数据文件、
// reset 隔离函数均为 feature 内部组成块，不进公开出口。
// 数据访问统一出口：GraphQL adapter（查询/纯文本写）+ REST adapter（文件上传/下载）。
...
} from './infrastructure/reference-document-http-adapter';
